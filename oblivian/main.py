...
def _handle_message(message_id: str, email_data: Dict[str, str]) -> None:
    ...
    if action == "refund" and isinstance(stripe_id, str) and stripe_id.startswith("pi_"):
        process_refund(stripe_id)
        # Pass the stripe_id directly into the function
        resolve_and_archive(message_id, sender_email, stripe_id)
        print(f"🌑 ECLIPSE: Refund executed and archived for {sender_email}.")
        return

    # THE INFINITE LOOP FIX: Mark as read so we never process it again
    mark_as_read(message_id)
    print(f"🌑 ECLIPSE: Ignored and marked read for {sender_email}.")
